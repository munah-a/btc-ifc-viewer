...
| T12 | High | ✅ (CI run 28767311085, PR #14) | `e2e/console-clean.spec.ts` test-timeout + `viewer.spec.ts` U4 error-state wait | Local `ci:local` passed 16/16 under CI=1 but GitHub failed 2 timeout-bound tests — a fast dev GPU box does not reproduce the 2-core runner's wall clock even with SwiftShader flags. (a) console-clean's own `test.setTimeout(10min)` was not CI-scaled and the full sweep needs >10min on the runner; (b) U4's corrupt-file `is-error` wait was 60s but on the slow runner web-ifc rejection / the app's 120s load timeout can exceed it. → console-clean `test.setTimeout` + waitForStatus CI-scaled (20min / 45s); U4 `ERROR_STATE_TIMEOUT = CI?150s:60s` (past the app's 120s ceiling). **Root cause = local↔CI parity: added opt-in `e2e/_cpu-throttle.ts` (`E2E_CPU_THROTTLE=<rate>` → CDP `setCPUThrottlingRate`) so `ci:local` can emulate the slow runner and catch this class before pushing.** |
