...
| W4 | Embed & sharing platform | /embed live on Vercel with upload→TTL→cleanup loop; GLB export; oEmbed; frame-ancestors; costs within the W4.3 envelope | 🔄 in progress (WO, wave/4-embed) — **build+local-test all code** (client + API + config, Blob/KV mocked); domain defaults to `btc-ifc-viewer-2.vercel.app`; **live Blob/KV provisioning + Vercel deploy deferred to a PO/user step** (account decision) |
