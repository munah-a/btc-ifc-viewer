...
| W3.7 | **i18n — EN + DE (C7, launch-blocking)** | all strings externalized to en/de catalogs; runtime language switch persisted; DE complete; brand-voice formats (DD.MM.YYYY, CHF); e2e language-switch test; new W4/W5 strings go through the catalog | ✅ WO-complete (wave/3.7-i18n): `core/i18n.ts` typed EN/DE catalogs (~190 keys) + `t()`/`hydrateI18n`/Intl helpers; EN⇄DE top-bar toggle persisted + in C8 state; DE complete; EN default unchanged (22 e2e pass); +i18n e2e; ci:local green. Awaiting PO merge. |
