...
│  │  ├─ persistence.ts       # versioned state schema, validate/apply (A7; +language C7) [done W1.6/W3.7]
│  │  ├─ i18n.ts              # typed EN/DE catalogs + t() + hydrateI18n + Intl date/number (C7) [done W3.7]
...
| W4 | Embed & sharing platform | /embed live on Vercel with upload→TTL→cleanup loop; GLB export; oEmbed; frame-ancestors; costs within the W4.3 envelope | ✅ **built + locally green** (WO, wave/4-embed): W4.1–W4.7 done; `ci:local` exit 0 (191 unit + SwiftShader e2e 30✓/1 skip/0 fail, 23.1m); C2 grep-verified; embed offline-clean; en+de. Storage mocked (InMemory) vs real (Blob+Upstash-Redis — **@vercel/kv sunset**) behind a lazy-imported adapter — **NOT provisioned, NOT deployed** (PO account step; checklist in Status Log). Domain default `btc-ifc-viewer-2.vercel.app`. |
