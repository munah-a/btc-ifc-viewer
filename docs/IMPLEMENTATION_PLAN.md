...
| W3 | Rebrand & responsive/a11y (+ W2-deferred decomposition) | New design shipped; U1–U11 fixed; both themes pass contrast; e2e updated + tablet test; **engine→`core/viewer-core.ts`, tools→`tools/*`, panels→`ui/*` built fresh on new DOM; viewer.ts → <~800-line orchestrator** | 🔄 in progress (WO agent, wave/3-rebrand) |
