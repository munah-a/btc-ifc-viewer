...
| 2026-07-13 | Claude (Fable 5) | **W4 share-hosting PROVISIONED + live-validated on production (user-approved).** (1) Deps: `@vercel/blob` + `@upstash/redis` installed; `optional-deps.d.ts` stubs removed; storage.ts boundary casts pin the real SDK surfaces (verified vs installed types) — PR #25 (CI 47s). (2) Resources: public Blob store `btc-ifc-embeds` (iad1, token auto-injected); **Upstash for Redis** via marketplace (`upstash/upstash-kv` — the product slug; user accepted the terms) injecting `KV_REST_API_URL/TOKEN`, which `Redis.fromEnv()` natively falls back to — no adapter change; `CRON_SECRET` + `BTC_OWNER_SALT` set in production (32-byte random, never displayed). NOTE: the marketplace installer drops unreviewed vendor agent-skills into `.agents/` — gitignored, flagged to the user. (3) **Latent W4 bug found by the FIRST live invocation:** every /api function crashed `FUNCTION_INVOCATION_FAILED` / `ERR_MODULE_NOT_FOUND` — Vercel runs the compiled TS as per-file Node ESM, which requires explicit extensions on relative imports; W4's extensionless `./_lib/*` specifiers never ran on a real deployment (unit/e2e used the in-memory adapter + mocked endpoints). Fixed by adding `.js` to all 16 relative specifiers — PR #27 (CI 50s), redeployed. (4) **Live production validation on viewer.bimtechconsulting.com:** cron fail-closed (401 no/wrong auth; Vercel sends the CRON_SECRET bearer on the daily 03:00 UTC schedule); oembed 200; unknown embed id 404 (Upstash round-trip proven); **full loop through the real UI:** school_str published → 938 KB `.frag` on the Blob CDN (200) + embed URL on the custom domain (200) + `Expires on 20.07.2026` (7-day TTL, Swiss format) → token DELETE → frag 404 AND metadata 404 (S3 delete-by-URL + S4 cache behavior confirmed). Residual: the first probe upload (`YiLCwibVWINWKr90`) outlived its lost token — expires 20.07 and the cron sweeps it (by design). Function body cap: Vercel accepts ~4.5 MB request bodies — fine for typical `.frag` sizes (school = 0.9 MB) but BELOW the 50 MB app cap; large-model publishing needs the `@vercel/blob` client-upload flow (W6 backlog item). |
