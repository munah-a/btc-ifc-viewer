 ...
 * there is no active issue (caller shows the "select an issue" prompt) and an
 * empty-list string is returned when the issue simply has no comments yet.
 */
export function buildIssueCommentsMarkup(issue: IssueRecord | undefined): string | null {
  if (!issue) return null;
  if (issue.comments.length === 0) return '<div class="comment-item">No comments</div>';
